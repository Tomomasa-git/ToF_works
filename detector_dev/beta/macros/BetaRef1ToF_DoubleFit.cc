...
#include "./Setting.h"
#include "./BetaRef1ToF_DoubleFit.hh"

////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////
BetaRef1ToF_DoubleFit::BetaRef1ToF_DoubleFit(){
	cout<<"BetaRef1ToF_DoubleFit::Constructer called"<<endl;
...
////////////////////////////////////////////////////////////////////////////////////
BetaRef1ToF_DoubleFit::~BetaRef1ToF_DoubleFit(){
	cout<<"BetaRef1ToF_DoubleFit::Destructer called"<<endl;
}	

////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////
double BetaRef1ToF_DoubleFit::Twlk_Correction( double x, double* parameter, double Coeff ){
	double Twlk_func=0.;
...
////////////////////////////////////////////////////////////////////////////////////
void BetaRef1ToF_DoubleFit::SetRoot( int RNum ){
	RunNum = RNum;
...
////////////////////////////////////////////////////////////////////////////////////
void BetaRef1ToF_DoubleFit::ImportPedestal( int PNum ){
	int i=0;
	double Val1, Val2;	
...
////////////////////////////////////////////////////////////////////////////////////
void BetaRef1ToF_DoubleFit::DefineCanv(){
	cout<<"=====   BetaRef1ToF_DoubleFit::DefineCanv ====="<<endl;
...
////////////////////////////////////////////////////////////////////////////////////
void BetaRef1ToF_DoubleFit::DefineHistFunc(){
	cout<<"=====   BetaRef1ToF_DoubleFit::DefineHistFunc ====="<<endl;
	...
	for(int i=0; i<NofMPPC; i++){
		...
		h_2D_FitTQ_Second[i] = new TH2D( Form("h_2D_FitTQ_Second[%d]",i), Form("h_2D_FitTQ_Second[%d]",i), 300, 0., 300., 100, -7000., 10500. );
		h_2D_DecTQ_Second[i] = new TH2D( Form("h_2D_DecTQ_Second[%d]",i), Form("h_2D_DecTQ_Second[%d]",i), 300, 0., 300., 100, -7000., 10500. );
		...
		f_Twlk_Second[i] = new TF1( Form("f_Twlk_Second[%d]", i), "[0]/sqrt(x-[1])+[2]", 0., 400.);
		...
		Set -> Setting_Hist2D( h_2D_DecTQ[i], "DecToF vs."+Label[i][0]+" w/ TWCx1"          , Label[i][0]+" [pC]", "DecToF [psec]", "Entry/bin", 1. );
		Set -> Setting_Hist2D( h_2D_CutTQ[i], "DecToF vs."+Label[i][0]+" w/ TWC and QDC cut", Label[i][0]+" [pC]", "DecToF [psec]", "Entry/bin", 1. );
		Set -> Setting_Hist2D( h_2D_FitTQ_Second[i], 
		                       "DecToF vs."+Label[i][0]+" w/ TWCx2", 
	                           Label[i][0]+" [pC]",
		                       "DecToF [psec]",
		                       "Entry/bin", 1. 
		                     );
		Set -> Setting_Hist2D( h_2D_DecTQ_Second[i],
		                       "DecToF vs."+Label[i][0]+" w/ TWCx2",
                               Label[i][0]+" [pC]",
		                       "DecToF [psec]",
		                       "Entry/bin", 1.
		...
		Set -> Setting_Func( f_Twlk_Second[i], 6, 1 );
	}
	...
	for(int i=0; i<NofCombination; i++){
		...
		h_2D_Factor[i+3]= new TH2D( Form("h_2D_Factor[%d]", i+3), Form("h_2D_Factor[%d]", i+3), 70, 0.0125, 1.7625, 70, 0.0125, 1.7625 );
		Gr_SigBest[i]        = new TGraph();
		Gr_SigBest_Second[i] = new TGraph();
	}
	...
	Set -> Setting_Hist2D( h_2D_Factor[3], "Factor tuning: ToF-1 vs. ToF-2 for Second Fit", "ToF-1 Factor", "ToF-2 Factor", "#sigma_{ToF} [psec]", 1.);
	Set -> Setting_Hist2D( h_2D_Factor[4], "Factor tuning: Ref-1 vs. ToF-1 for Second Fit", "Ref-1 Factor", "ToF-1 Factor", "#sigma_{ToF} [psec]", 1.);
	Set -> Setting_Hist2D( h_2D_Factor[5], "Factor tuning: Ref-1 vs. ToF-2 for Second Fit", "Ref-1 Factor", "ToF-2 Factor", "#sigma_{ToF} [psec]", 1.);
	...
	Set -> Setting_Graph( Gr_SigBest_Second[0], "Factor tuning: ToF-1 vs. ToF-2 for Second Fit", "ToF-1 Factor", "ToF-2 Factor", 1, 1, 62, 6, 29 );
	Set -> Setting_Graph( Gr_SigBest_Second[1], "Factor tuning: Ref-1 vs. ToF-1 for Second Fit", "Ref-1 Factor", "ToF-1 Factor", 1, 1, 62, 6, 29 );
	Set -> Setting_Graph( Gr_SigBest_Second[2], "Factor tuning: Ref-1 vs. ToF-2 for Second Fit", "Ref-1 Factor", "ToF-2 Factor", 1, 1, 62, 6, 29 );
...
////////////////////////////////////////////////////////////////////////////////////
void BetaRef1ToF_DoubleFit::FillHist(){
	cout<<"=====   BetaRef1ToF_DoubleFit::FillHist   ====="<<endl;
...
////////////////////////////////////////////////////////////////////////////////////
void BetaRef1ToF_DoubleFit::GetQDCCut(){
	cout<<"=====   BetaRef1ToF_DoubleFit::GetQDCCut   ====="<<endl;
	for(int i=0; i<NofMPPC; i++){
...
////////////////////////////////////////////////////////////////////////////////////
void BetaRef1ToF_DoubleFit::TwlkFit(){
	cout<<"=====   BetaRef1ToF_DoubleFit::TwlkFit   ====="<<endl;
	...
	for(int i=0; i<NofMPPC; i++){
		...
		for(int j=0; j<NDiv[i]; j++){
			...
			if( TwlkFitNDF>3. ){
				Gr_fit[0][i] -> SetPoint( NFitPoint, xCenter, f_gaus->GetParameter(1) );
				Gr_fit[0][i] -> SetPointError( NFitPoint, 0., f_gaus->GetParameter(2) );
				NFitPoint++;
			}else;
		}
		Ca[2]->cd(i+1);
		Gr_fit[0][i]->Draw("sameP");
		Gr_fit[0][i]->Fit( f_Twlk[i], "", "", TwlkFitMin[i], TwlkFitMax[i] );
...
////////////////////////////////////////////////////////////////////////////////////
void BetaRef1ToF_DoubleFit::TwlkSerchBestFactorToF1vsToF2(){
	cout<<"=====   BetaRef1ToF_DoubleFit::TwlkSerchBestFactorToF1vsToF2   ====="<<endl;
	TwlkFactor[0] = 1.;
	for(int i=0; i<70; i++){
		for(int j=0; j<70; j++){
...
////////////////////////////////////////////////////////////////////////////////////
void BetaRef1ToF_DoubleFit::TwlkSerchBestFactorRef1vsToF1(){
	cout<<"=====   BetaRef1ToF_DoubleFit::TwlkSerchBestFactorRef1vsToF1   ====="<<endl;
	for(int i=0; i<70; i++){
		for(int j=0; j<70; j++){
...
////////////////////////////////////////////////////////////////////////////////////
void BetaRef1ToF_DoubleFit::TwlkSerchBestFactorRef1vsToF2(){
	cout<<"=====   BetaRef1ToF_DoubleFit::TwlkSerchBestFactorRef1vsToF2   ====="<<endl;
	for(int i=0; i<70; i++){
		for(int j=0; j<70; j++){
			TwlkFactor[0] = 0.025+0.025*i;
			TwlkFactor[2] = 0.025+0.025*j;
...
////////////////////////////////////////////////////////////////////////////////////
void BetaRef1ToF_DoubleFit::TwlkDecToF(){
	cout<<"=====   BetaRef1ToF_DoubleFit::TwlkDecToF   ====="<<endl;
...
////////////////////////////////////////////////////////////////////////////////////
void BetaRef1ToF_DoubleFit::TwlkFit_Second(){
	cout<<"=====   BetaRef1ToF_DoubleFit::TwlkFit_Second   ====="<<endl;
	f_Twlk[1]->SetParameters(1.E+5, 10., 0. );
	f_Twlk[2]->SetParameters(1.E+5, 10., 0. );
	...
	for(int i=0; i<Entry; i++){
		...
		if(
			...
			h_2D_FitTQ_Second[0] -> Fill(0.1*QDC[0]-0.1*Pede_Cnt_Val[0], dectof);
		}else;
		if(
			   0.1*QDC[ChID[0]]>QCut[0][0]
			&& 0.1*QDC[ChID[0]]<QCut[0][1]
			...
			h_2D_FitTQ_Second[1] -> Fill(0.1*QDC[2]-0.1*Pede_Cnt_Val[1], dectof);
			h_2D_FitTQ_Second[2] -> Fill(0.1*QDC[3]-0.1*Pede_Cnt_Val[2], dectof);
		}else;
	}
	...
	for(int i=0; i<2; i++){
		Ca[4]->cd(i+1);
		gPad->SetLogz(1);
		h_2D_FitTQ_Second[i+1]->Draw("colz");
		NFitPoint=0;
		for(int j=0; j<NDiv[i+1]; j++){
			Ca_dummy[i]->cd();
			Div_min = j*( (TwlkFitMax[i+1]-TwlkFitMin[i+1])/NDiv[i+1])+TwlkFitMin[i+1];
			Div_max = (j+1)*( (TwlkFitMax[i+1]-TwlkFitMin[i+1])/NDiv[i+1])+TwlkFitMin[i+1];
			TH1D *h_projection=(TH1D*)h_2D_FitTQ_Second[i+1]->ProjectionY( "", Div_min, Div_max )->Clone();
			par_tmp3 = h_projection->GetBinCenter( h_projection->GetMaximumBin() );
			h_projection->Fit( f_gaus, "Q", "", par_tmp3-2000., par_tmp3+2000.);
			h_projection->Draw("");
			TwlkFitNDF = f_gaus->GetNDF();
			xCenter = 0.5*(Div_max-Div_min)+Div_min;
			if( TwlkFitNDF>3. ){
				Gr_fit[1][i+1] -> SetPoint( NFitPoint, xCenter, f_gaus->GetParameter(1) );
				Gr_fit[1][i+1] -> SetPointError( NFitPoint, 0., f_gaus->GetParameter(2) );
				NFitPoint++;
			}else;
		}
		Ca[4]->cd(i+1);
		Gr_fit[1][i+1]->Draw("sameP");
		Gr_fit[1][i+1]->Fit( f_Twlk_Second[i+1], "", "", TwlkFitMin[i+1], TwlkFitMax[i+1] );
//		h_2D_FitTQ[i]->Fit( f_Twlk[i], "", "", TwlkFitMin[i], TwlkFitMax[i] );
		for(int j=0; j<3; j++){TwlkSecondPar_Val[i][j]=f_Twlk_Second[i]->GetParameter(j);}
	}
}

////////////////////////////////////////////////////////////////////////////////////
void BetaRef1ToF_DoubleFit::TwlkSerchBestFactorToF1vsToF2_Second(){
	cout<<"=====   BetaRef1ToF_DoubleFit::TwlkSerchBestFactorToF1vsToF2_Second   ====="<<endl;	
	for(int i=0; i<70; i++){
		for(int j=0; j<70; j++){
			TwlkSecondFactor[1] = 0.025+0.025*i;
			TwlkSecondFactor[2] = 0.025+0.025*j;
			...
			for(int k=0; k<Entry; k++){
				...
				dectof = 35.*( TDC[ChID[0]] - 0.5*(TDC[ChID[1]]+TDC[ChID[2]]) )
					     ...
					     -Twlk_Correction( 0.1*QDC[3]-0.1*Pede_Cnt_Val[2], TwlkPar_Val[2]       , TwlkFactor[2]       )
				         -ToF_Adj
				         -Twlk_Correction( 0.1*QDC[2]-0.1*Pede_Cnt_Val[1], TwlkSeconedPar_Val[1], TwlkSecondFactor[1] )
					     -Twlk_Correction( 0.1*QDC[3]-0.1*Pede_Cnt_Val[2], TwlkSeconedPar_Val[2], TwlkSecondFactor[2] );
				if(
					   0.1*QDC[ChID[0]]>0.1*Pede_Cnt_Val[0]
					&& 0.1*QDC[ChID[1]]>0.1*Pede_Cnt_Val[1]
					&& 0.1*QDC[ChID[2]]>0.1*Pede_Cnt_Val[2]
			...
			if( TwlkSecond_SigVal_tmp<TwlkSecond_SigVal_Best[0] ){
				TwlkSecond_SigVal_Best[0] = TwlkSecond_SigVal_tmp;
				TwlkSecond_SigErr_Best[0] = TwlkSecond_SigErr_tmp;
				TwlkSecond_FactorBestSigma[1] = TwlkSecondFactor[1];
				TwlkSecond_FactorBestSigma[2] = TwlkSecondFactor[2];
				cout<<"Second Twlk factor ToF-1 and ToF-2 >> "<<TwlkSecond_FactorBestSigma[1]<<",  "<<TwlkSecond_FactorBestSigma[2]<<":  "<<"Sigma_TOF = "<<TwlkSecond_SigVal_Best[0]<<" psec"<<endl;
			}else;
			h_2D_Factor[3] -> Fill( TwlkSecondFactor[1], TwlkSecondFactor[2], TwlkSecond_SigVal_tmp );
		}
	}
	Gr_SigBest[0]  -> SetPoint( 0, Twlk_FactorBestSigma[1] , Twlk_FactorBestSigma[2]  );
	TextBestSig[0] = Form("Best Factor = (%.3lf, %.3lf)", Twlk_FactorBestSigma[1], Twlk_FactorBestSigma[2]);
	h_2D_Factor[3] -> SetMinimum( Twlk_SigVal_Best[0] );
	TwlkFactor[1] = Twlk_FactorBestSigma[1];
	TwlkFactor[2] = Twlk_FactorBestSigma[2];
	Twlk_SigVal_Best[1] = Twlk_SigVal_Best[0];
...
////////////////////////////////////////////////////////////////////////////////////
void BetaRef1ToF_DoubleFit::TwlkSerchBestFactorRef1vsToF1_Second(){
	cout<<"=====   BetaRef1ToF_DoubleFit::TwlkSerchBestFactorRef1vsToF1_Second   ====="<<endl;	
}

////////////////////////////////////////////////////////////////////////////////////
void BetaRef1ToF_DoubleFit::TwlkSerchBestFactorRef1vsToF2_Second(){
	cout<<"=====   BetaRef1ToF_DoubleFit::TwlkSerchBestFactorRef1vsToF2_Second   ====="<<endl;	
}

////////////////////////////////////////////////////////////////////////////////////
void BetaRef1ToF_DoubleFit::DrawHist(){
	cout<<"=====   BetaRef1ToF_DoubleFit::DrawHist   ====="<<endl;
...
////////////////////////////////////////////////////////////////////////////////////
void BetaRef1ToF_DoubleFit::SaveHist( int NSave){
	cout<<"=====   BetaRef1ToF_DoubleFit::SaveHist   ====="<<endl;
	if(NSave==0){
		NofSaveCanv = NofCanv;
	...
	figure       = Form("run%03d_BetaRef1ToF_DoubleFit_01.pdf", RunNum);
...
////////////////////////////////////////////////////////////////////////////////////
void BetaRef1ToF_DoubleFit::ExportTwlk(){
	cout<<"=====   BetaRef1ToF_DoubleFit::ExportTwlk   ====="<<endl;
	string ExParam = Form("paramTwlkDoubleFit_run%03d_00.dat", RunNum);
...
int main( int argc, char** argv ){
	...
	TApplication *theApp;
	BetaRef1ToF_DoubleFit *analysis;

	theApp = new TApplication("App", &argc, argv );
	analysis = new BetaRef1ToF_DoubleFit();
